...
PROJECT_ROOT = PACKAGE_DIR.parent.parent  # src
ASSETS_DIR = PROJECT_ROOT / "assets"  # assets
...
def _normalize_friction_mode(mode: Optional[str]) -> str:
    value = "max" if mode is None else mode.strip().lower()
    if value not in {"max", "min"}:
        raise ValueError("Mode de frottement invalide (max ou min).")
    return value


def _validate_mu_input(value: Optional[float], label: str) -> Optional[float]:
    if value is None:
        return None
    if value < 0 or value > 1:
        raise ValueError(f"{label} doit etre compris entre 0 et 1.")
    return value
...
class Vis:
    ...
    mu_sous_rondelle: float = field(init=False, default=0.0)
    last_denominator: float = field(init=False, default=0.0)
    force_resistance_glissement: float = field(init=False, default=0.0)
    ...
    def effort_serrage(
        ...
        friction_mode: str = "max",
        mu_filet_manual: Optional[float] = None,
        mu_sous_tete_manual: Optional[float] = None,
        ...
        friction_mode = _normalize_friction_mode(friction_mode)
        mu_filet_manual = _validate_mu_input(mu_filet_manual, "Frottement des filets")
        mu_sous_tete_manual = _validate_mu_input(mu_sous_tete_manual, "Frottement sous tete")
        ...
        friction_filets = self._lookup_friction_coefficients(self.mat_vis, self.mat_body, friction_mode)
        if self.mat_sous_tete == self.mat_body:
            friction_sous_tete = friction_filets
        else:
            friction_sous_tete = self._lookup_friction_coefficients(self.mat_vis, self.mat_sous_tete, friction_mode)

        if mu_filet_manual is not None:
            self.mu_filet = mu_filet_manual
        else:
            self.mu_filet = (
        ...
        if mu_sous_tete_manual is not None:
            self.mu_sous_tete = mu_sous_tete_manual
        else:
            self.mu_sous_tete = (
        ...
        # Gestion des rondelles
        self.mu_sous_rondelle = self.mu_sous_tete
        if has_washer:
            mat_rondelle_norm = self._normalize_material(mat_rondelle)
            mat_sous_rondelle_norm = self._normalize_material(mat_sous_rondelle or self.mat_sous_tete)
            friction_rondelle = self._lookup_friction_coefficients(mat_rondelle_norm, mat_sous_rondelle_norm, friction_mode)
            self.mu_sous_rondelle = (
                friction_rondelle["lubricated_tete"]
                if self.is_sous_tete_lubrified
                else friction_rondelle["dry_tete"]
            )
        ...
        self.force_resistance_glissement = (self.mu_sous_rondelle if has_washer else self.mu_sous_tete) * effort
    ...
    def _lookup_friction_coefficients(self, mat_vis: str, mat_body: str, mode: Literal["max", "min"] = "max") -> dict[str, float]:
        if not FROTTEMENT_FILE.exists():
            raise FileNotFoundError(f"Le fichier {FROTTEMENT_FILE} est introuvable.")
        mode_value = _normalize_friction_mode(mode)
        suffix = "min" if mode_value == "min" else "max"
        with FROTTEMENT_FILE.open("r", encoding="utf-8", newline="") as csvfile:
            ...
            for row in reader:
                ...
                if row_mat_vis == mat_vis and row_mat_body == mat_body:
                    try:
                        return {
                            "dry_filet": self._parse_float(row[f"Dry_{suffix}_filet"]),
                            "dry_tete": self._parse_float(row[f"Dry_{suffix}_tete"]),
                            "lubricated_filet": self._parse_float(row[f"Lubricated_{suffix}_filet"]),
                            "lubricated_tete": self._parse_float(row[f"Lubricated_{suffix}_tete"]),
                        }
                    except (KeyError, ValueError) as exc:
                        raise ValueError(
...
def dimensionner(
    ...
    *,
    ...
    mu_filet_manual = _validate_mu_input(mu_filet_manual, "Frottement des filets")
    mu_sous_tete_manual = _validate_mu_input(mu_sous_tete_manual, "Frottement sous tete")
    mat_rondelle_effectif = mat_rondelle
    mat_sous_rondelle_effectif = mat_sous_rondelle
    if has_washer:
        if mat_rondelle_effectif is None:
            mat_rondelle_effectif = mat_body
        if mat_sous_rondelle_effectif is None:
            mat_sous_rondelle_effectif = mat_body
    ...
    def _evaluate(diam: float, lub: bool) -> None:
        ...
        for head_type, diam_tete in candidates:
            ...
            _effort_max = vis.effort_serrage(
                ...
                mu_sous_tete_manual=mu_sous_tete_manual,
                has_washer=has_washer,
                mat_rondelle=mat_rondelle_effectif,
                mat_sous_rondelle=mat_sous_rondelle_effectif,
            )
